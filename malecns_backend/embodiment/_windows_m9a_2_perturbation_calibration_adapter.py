"""Live FlyGym/MuJoCo boundary for M9A-2; contains no neural-runtime import."""
...
from . import m9a_2_perturbation_calibration as m9a
...
def _modules() -> tuple[Any, Any, Any, dict[str, Any]]:
    ...
    if versions != {"flygym": "1.2.1", "mujoco": "3.2.7"}:
        raise RuntimeError(f"M9A-2 requires FlyGym 1.2.1 and MuJoCo 3.2.7; found {versions}")
...
def run_windows() -> dict[str, Any]:
    if m9a.REPORT_PATH.exists() or m9a.MANIFEST_PATH.exists():
        raise FileExistsError("refusing to overwrite M9A-2 calibration evidence")
    ...
    for magnitude in m9a.CANDIDATE_FORCE_NATIVE:
        ...
        path = m9a.OUTPUT_DIR / f"candidate_{magnitude:.6f}_raw.npz"
    ...
    report = {"schema": m9a.SCHEMA, "status": "COMPLETE", "classification": "EXTERNAL_PHYSICAL_PERTURBATION_CALIBRATED",
        "claim_limit": "M9A-2 does not demonstrate balance, postural control, reflexes, biological function, or neural recovery.",
