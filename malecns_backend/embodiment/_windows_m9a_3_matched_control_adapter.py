...
from . import m9a_3_matched_control_calibration as m9a
...
def _reduce_pair(np: Any, p: Mapping[str, Any], c: Mapping[str, Any], magnitude: float) -> dict[str, Any]:
    ...
    early=t<=750; catastrophic=bool(np.any(((p["root_position"][:,2] <= initial[2]*.5)|(c["root_position"][:,2] <= initial[2]*.5)
        ...
        |(_norm(np,p["linear_velocity"])>10)|(_norm(np,c["linear_velocity"])>10)) & early))
...
def windows_preflight() -> dict[str, Any]:
    ...
    return {"schema":m9a.SCHEMA,"status":"PREFLIGHT_PASS","environment":env,"force_body":{k:v for k,v in body.items() if k!="all_body_names"},
