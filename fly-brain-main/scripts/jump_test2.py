    ...
    for leg in LEGS:
        for sd in SIDES:
            ...
            for j in J:
                ...
                lo, hi = rng[(leg, sd, j)]; d.ctrl[act[(leg, sd, j)]] = np.clip(0.4 * (off + q), lo, hi)
            d.ctrl[act[(leg, sd, 'adh')]] = 1.0 if ((phi + adhph) % (2 * np.pi)) < 2 * np.pi * duty else 0.0
def trial(prog, walk_ms):
    mujoco.mj_resetData(m, d); phase = 0.0; dtms = m.opt.timestep * 1000
    for s in range(int(walk_ms / dtms)): phase += 2 * np.pi * 7 * m.opt.timestep; walk_ctrl(phase); mujoco.mj_step(m, d)
...
res = []
for push, f2, t2, f3, f1, fly in itertools.product([10, 20, 30], [0.7, 1.0], [0.5, 1.0], [0.0, 0.4], [0.0, 0.5], [80]):
    prog = dict(push=push, f2=f2, t2=t2, f3=f3, f1=f1, fly=fly); ok = 0; worst = 1; hs = []
    for w in [100, 137, 174, 211, 248, 285, 322, 359]:
        mu, fu, h = trial(prog, w); ok += fu > 0.8; worst = min(worst, mu); hs.append(h)
    res.append((ok, float(np.min(hs)), worst, prog))
res.sort(key=lambda r: (r[0] == 8, r[1]), reverse=True)
for r in res[:6]: print(f'upright {r[0]}/8, min hop height {r[1]*10:.2f} mm, worst tilt up={r[2]:.2f}', r[3])
