    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                // The noise chooses between two genuine albedo samples. It is not a
                // dark overlay and therefore cannot drive the base color toward zero.
                float blend = smoothstep(0.30, 0.70, macro);
                blend = _GroundRenderingMode >= 1.5 ? blend : 0.0;
                ...
                // A strength of zero is an exact no-op. The default multiplier is
                // 0.97--1.03, preserving hue as well as average brightness.
                if (_GroundRenderingMode >= 2.5)
                    albedo *= lerp(1.0h - (half)_MacroVariationStrength,
                                   1.0h + (half)_MacroVariationStrength, (half)macro);

                // Diagnostic mode 0 bypasses lighting and every non-albedo map.
                ...
                // AO is passed to URP's occlusion input and never multiplied into albedo.
